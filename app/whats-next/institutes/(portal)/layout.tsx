...
const NAV = [
  { href: `${WHATS_NEXT_BASE}/institutes/dashboard`, label: "Dashboard" },
  { href: `${WHATS_NEXT_BASE}/institutes/dashboard#students`, label: "Students" },
  { href: `${WHATS_NEXT_BASE}/institutes/reports`, label: "Reports" },
  { href: `${WHATS_NEXT_BASE}/institutes/results`, label: "Results" },
];
  ...
  if (allowed === false) {
    return (
      <div className="min-h-screen bg-transparent flex flex-col items-center justify-center px-4">
        ...
        <Link
          href={`${WHATS_NEXT_BASE}/institutes/register`}
    ...
    <div className="min-h-screen bg-transparent flex">
      <aside className="w-56 bg-white/80 backdrop-blur-md shadow-xl border-r border-white/60 flex flex-col shrink-0">
        <div className="p-4 border-b border-white/60">
          <Link href={`${WHATS_NEXT_BASE}/institutes/dashboard`} className="font-semibold text-gray-900">
            Institute
          </Link>
        </div>
        <nav className="p-2 flex-1">
          {NAV.map((item) => {
            const isActive =
              item.href === pathname ||
              (item.href === `${WHATS_NEXT_BASE}/institutes/dashboard` && pathname?.startsWith(`${WHATS_NEXT_BASE}/institutes/dashboard`));
            return (
              <Link
                key={item.href}
                href={item.href}
